...
function getProductInfo(productId, shippingCost) {
    ...
    $.get(requestUrl, function (prodJson){
        let productName = prodJson.name;
        let mainImagePath = contextPath.substring(0, contextPath.length - 1) + prodJson.imagePath;
        let productCost = $.number(prodJson.cost, 2);
        let productPrice = $.number(prodJson.price, 2);

        let htmlCode = generateProductHtmlCode(productId, productName, mainImagePath, productCost, productPrice, shippingCost)
...
function generateProductHtmlCode(productId, productName, mainImagePath, productCost, productPrice, shippingCost) {
    ...
    htmlCode = `
          ...
          <div id="'blankLine' + ${nextCount}" class="row">&nbsp;</div>`;
